...
export class TopbarComponent {
  ...
  newNotify: number = 0;
  readNotify: number = 0;
  notifications: any[] = []; // Ensure this property is correctly defined
      ...
      next: (response: any) => {
        if (response && response.data) {
          ...
          this.notificationList.forEach((notification: any) => {
            this.totalNotify += 1; // Example increment
            if (notification.title === 'New') {
              this.newNotify += 1; // Example increment
            } else {
              this.readNotify += 1; // Example increment
            }
          });
        } else {
